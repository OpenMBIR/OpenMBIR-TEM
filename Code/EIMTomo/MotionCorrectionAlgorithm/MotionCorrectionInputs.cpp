...
MotionCorrectionInputs::MotionCorrectionInputs()
{
...
//
// -----------------------------------------------------------------------------
int MotionCorrectionInputs::CI_ParseInput(int argc,char **argv,CommandLineInputs* Input)
{
  if ( NULL == Input)
  {
    printf("The MotionCorrectionInputs pointer was null. Returning early.\n");
    return -1;
  }
  ...
  if (argc < 2)
  {
    std::cout << "MotionCorrection Command Line Version " << cmd.getVersion() << std::endl;
...
void MotionCorrectionInputs::CI_ReadParameterFile(FILE *Fp, CommandLineInputs* ParsedInput, Sino* Sinogram, Geom* Geometry)
{
  char temp[20];
  int16_t i;
  while (!feof(Fp))
...
void MotionCorrectionInputs::CI_InitializeSinoParameters(Sino* Sinogram, CommandLineInputs* ParsedInput)
{
  int16_t i, j, k;
  FILE* Fp;
...
void MotionCorrectionInputs::CI_InitializeGeomParameters(Sino* Sinogram, Geom* Geometry, CommandLineInputs* ParsedInput)
{
  FILE* Fp;
  uint16_t i, j, k;
